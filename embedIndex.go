...
<body>
  <div class="container">
    ...
    <a href="/download/"><button>View files</button></a>
    <a class="button button-outline" href="/downloadself/"><button>Get NoUSB</button></a>
    <hr>
    <h1>Documentation</h1>
  ...
  <style type="text/css">
    ...
    hr {
      color: none; border: none; outline: none;
      background-color: rgb(30,30,30);
      height: 1px;
    }
    code { display: block; width: 100%; }
    code, ul#externalFileList { background-color: rgb(240,240,240); }
    input, button { padding: 0.25rem; border-radius: 0.25rem; height: 2rem; padding: 0 0.5rem; margin: 0.25rem; }
    input { border: 1px solid rgb(200,200,200); }
    input:focus { border-color: rgb(30,30,30); }
    button, hr { background-color: rgb(30,30,30); border: none; outline: none; }
  ...
  function getExternalFiles() {
    ...
    var url = "/api/external/files/?url=" + externalServerUrl;

    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        var files = JSON.parse(this.responseText);
        var ul = document.getElementById("externalFileList");
        for (var i = 0; i < files.length; i++) {
          var li = document.createElement("li");
          var a = document.createElement("a");
          a.setAttribute("href", "http://"+externalServerUrl+"/download/"+files[i]);
          a.innerText = files[i];
          li.appendChild(a);
          ul.appendChild(li);
        }
